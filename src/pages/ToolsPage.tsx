...
import { 
  ...
  batteries,
  ores,
  optimizeBatteryCombo  // Fonction exportée depuis thrustersData.ts
...
// Composant de vérification de cohérence (à compléter selon vos données extraites)
const GameConsistencyCheck = ({ type, id }: { type: 'thruster' | 'battery'; id: string }) => {
  ...
  useEffect(() => {
    // Ici, vous pourrez comparer vos données locales avec celles extraites du jeu
    setConsistency('full');
  }, [type, id]);
  return (
...
// Fonction globale de sélection de combinaison de thrusters avec +10% de marge sur la poussée requise
const selectThrusterCombination = (
  available: { key: string; thruster: ThrusterData }[],
  R: number,
  atmosphereValue: number
): { combination: { key: string; count: number }[] } => {
  const effectiveR = R * 1.1; // ajout de 10% de marge
  // 1. Si un thruster seul fournit suffisamment
  ...
  // 2. Essayons des combinaisons hybrides
  ...
  // 3. Sinon, combiner un thruster large et compléter avec small
  if (large.length > 0) {
  ...
  // 4. Sinon, choisir le meilleur global
...
const SpaceCalcPage: React.FC = () => {
  ...
  const [targetAutonomy, setTargetAutonomy] = useState<number>(1); // Autonomie cible en heures
  ...
  // Calcul de la combinaison d'un axe (lateral=true force l'utilisation de petits thrusters)
  ...
  const calculateThrusters = (e: React.FormEvent): void => {
    ...
    // Calcul des batteries en tenant compte de l'autonomie cible
    const requiredPower = totalPowerConsumed / 1e6; // en MW
    const requiredEnergy = requiredPower * targetAutonomy; // en MWh
    // Correction dans optimizeBatteryCombo : s = ceil(max(requiredEnergy - l * large.maxStoredPower, 0) / small.maxStoredPower)
    const optimalCombo = optimizeBatteryCombo(requiredPower, requiredEnergy);
    const finalCombo = {
      large: Math.max(Math.ceil(optimalCombo.large * (safetyMargin / 100)), 0),
      small: Math.max(Math.ceil(optimalCombo.small * (safetyMargin / 100)), 0)
    };
    const batteryReqs = {
      ...
      flightTime: (finalCombo.large * batteries.largeBattery.maxStoredPower + finalCombo.small * batteries.smallBattery.maxStoredPower)
  ...
  const generateSummary = (): string => {
    ...
    summary += `• Masse Naturelle : ${naturalMassRatio}% | Safety Margin : ${safetyMargin}% | Autonomie Cible : ${targetAutonomy} h\n\n`;
          ...
          <form onSubmit={calculateThrusters}>
            ...
            <div className="input-group">
              ...
              <input 
                ...
                max="2" 
                step="0.1" 
                value={targetAutonomy} 
                onChange={(e) => setTargetAutonomy(parseFloat(e.target.value))}
              />
